...
import './style.css'
import {distinctUntilChanged, filter, from, fromEvent,  map, scan, switchMap} from 'rxjs'
...
const searchbarInput$ = searchbarEvent$
  .pipe(
...
const filteredRugstock$ = searchbarInput$
  .pipe(
    switchMap((input) => rugstockObservable$
      .pipe(
...
searchbarInput$
  .pipe(map(userQuery => userQuery.trim().toLowerCase()), distinctUntilChanged())
    ...
    from(children)
      .pipe(filter(child =>
        !child
          .innerText
          .toLocaleLowerCase()
          .includes(distinctUserQuery
          .toLocaleLowerCase()))).subscribe(child => productWrapper.removeChild(child))
